#include "website.h"
// To provide port number and timeout
#include "config.h"

// Used for request header
void WebSite::start_server() {
...
void WebSite::stop() { client.stop(); }

void WebSite::serve(Data &new_data) {
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            if (header.indexOf("GET /data") >= 0) {
              ...
              client.print(new_data.wind_speed);
              client.print(",");
              client.print("\"wind_direction\":");
              client.print(new_data.wind_direction);
              client.print(",");
              ...
              client.println("<h3> Station IP :  " +
                             String(WiFi.localIP().toString()) + "</h3>");
              client.println("<p id='temperature'>Temperature: " +
                             String(new_data.temperature) + " (degree) C</p>");
              client.println("<p id='humidity'>Humidity: " +
                             String(new_data.humidity) + " %</p>");
              client.println("<p id='wind_speed'>Wind Speed: " +
                             String(new_data.wind_speed) + " kph </p>");
              client.println("<p id='wind_direction'>Wind Direction: " +
                             String(new_data.wind_direction) +
                             "(degrees) </p>");
              client.println("<p id='rain_volume'>Total Rain Fall: " +
              ...
              client.println(
                  "      document.getElementById('rain_volume').innerHTML = "
                  "'Total Rain Fall: ' + data.rain_volume + ' mm';");
              client.println(
                  "      document.getElementById('light_intensity').innerHTML "
